...
async function initDatabase() {
    ...
    // Добавляем недостающие колонки (игнорируем ошибки, если колонки уже есть)
    try {
        await db.exec(`ALTER TABLE users ADD COLUMN total_voice_minutes INTEGER DEFAULT 0`);
    } catch (e) { /* колонка уже существует */ }
    
    try {
        await db.exec(`ALTER TABLE users ADD COLUMN voice_minutes_today INTEGER DEFAULT 0`);
    } catch (e) { /* колонка уже существует */ }
    
    try {
        await db.exec(`ALTER TABLE users ADD COLUMN last_voice_reset INTEGER`);
    } catch (e) { /* колонка уже существует */ }
    ...
    await db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_pending_marriages_target ON pending_marriages(target_id);
    `);
